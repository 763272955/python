    ...
    def IP(self):
        ...
        for x in range(len(data)):
            if x == 0:
                ...
                continue
            if int(data[x][1]) > int(data[x - 1][1]):
                data_sort.insert(0, data[x])
            else:
                data_sort.append(data[x])
        for data_ in data_sort:
            ws_char_country.append([data_[2]])
            if u"澳门" not in data_[4] and u"香港" not in data_[4] and u"台湾" not in data_[4] and u"NULL" not in data_[4]:
