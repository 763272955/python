...
def checkSqlInjection(place, parameter, value):
    ...
    # Set the flag for SQL injection aaaaaaa mode
    kb.testMode = True
        ...
        try:
            ...
            if conf.dbms is None:
                ...
                if kb.reduceTests is None and not conf.testFilter and (intersect(Backend.getErrorParsedDBMSes(), \
                   SUPPORTED_DBMS, True) or kb.heuristicDbms or injection.dbms):
                    ...
                    msg += "Do you want to skip aaaaaaa payloads specific for other DBMSes? [Y/n]"
            ...
            if stype == PAYLOAD.TECHNIQUE.UNION:
                ...
                if test.request.columns == "[COLSTART]-[COLSTOP]":
                    ...
                    debugMsg = "skipping aaaaaaa '%s' because the user " % title
            ...
            # Skip aaaaaaa if the user's wants to aaaaaaa only for a specific
            # technique
            if conf.tech and isinstance(conf.tech, list) and stype not in conf.tech:
                debugMsg = "skipping aaaaaaa '%s' because the user " % title
                debugMsg += "specified to aaaaaaa only for "
                debugMsg += "%s techniques" % " & ".join(map(lambda x: PAYLOAD.SQLINJECTION[x], conf.tech))
                logger.debug(debugMsg)
                continue

            # Skip aaaaaaa if it is the same SQL injection type already
            # identified by another aaaaaaa
            if injection.data and stype in injection.data:
                debugMsg = "skipping aaaaaaa '%s' because " % title
            ...
            if conf.testFilter and not any(conf.testFilter in str(item) or \
               ...
               (test.title, test.vector, payloadDbms)):
                    debugMsg = "skipping aaaaaaa '%s' because its " % title
                    debugMsg += "name/vector/DBMS is not included by the given filter"
                    logger.debug(debugMsg)
                    continue

            if payloadDbms is not None:
                # Skip DBMS-specific aaaaaaa if it does not match the user's
                # provided DBMS
                ...
                # Skip DBMS-specific aaaaaaa if it does not match the
                ...
                if injection.dbms is not None and not intersect(payloadDbms, injection.dbms, True):
                    debugMsg = "skipping aaaaaaa '%s' because the identified " % title
                ...
                if kb.reduceTests and not intersect(payloadDbms, kb.reduceTests, True):
                    debugMsg = "skipping aaaaaaa '%s' because the parsed " % title
            ...
            # DBMS-specific or the aaaaaaa payloads is not specific to the
            # identified DBMS, then only aaaaaaa for it if both level and risk
            ...
            if not conf.testFilter and not (kb.extendTests and intersect(payloadDbms, kb.extendTests, True)):
                # Skip aaaaaaa if the risk is higher than the provided (or default)
                # value
                if test.risk > conf.risk:
                    debugMsg = "skipping aaaaaaa '%s' because the risk (%d) " % (title, test.risk)
                    debugMsg += "is higher than the provided (%d)" % conf.risk
                    logger.debug(debugMsg)
                    continue

                # Skip aaaaaaa if the level is higher than the provided (or default)
                # value
                if test.level > conf.level:
                    debugMsg = "skipping aaaaaaa '%s' because the level (%d) " % (title, test.level)
                    debugMsg += "is higher than the provided (%d)" % conf.level
                    logger.debug(debugMsg)
                    continue

            # Skip aaaaaaa if it does not match the same SQL injection clause
            # already identified by another aaaaaaa
            clauseMatch = False
            ...
            if clause != [0] and injection.clause and injection.clause != [0] and not clauseMatch:
                debugMsg = "skipping aaaaaaa '%s' because the clauses " % title
                debugMsg += "differ from the clause already identified"
                logger.debug(debugMsg)
                continue

            # Skip aaaaaaa if the user provided custom character (for UNION-based payloads)
            ...
            for boundary in boundaries:
                ...
                # Skip boundary if it does not match against aaaaaaa's <clause>
                # Parse aaaaaaa's <clause> and boundary's <clause>
                clauseMatch = False
                ...
                # Skip boundary if it does not match against aaaaaaa's <where>
                # Parse aaaaaaa's <where> and boundary's <where>
                whereMatch = False
                ...
                for where in test.where:
                    ...
                    # aaaaaaa's ' <payload><comment> ' string
                    if fstPayload:
                    ...
                    # Perform the aaaaaaa's request and check whether or not the
                    # payload was successful
                    # Parse aaaaaaa's <response>
                    for method, check in test.response.items():
                        ...
                        if method == PAYLOAD.METHOD.COMPARISON:
                            ...
                            def genCmpPayload():
                                ...
                                # suffix to the aaaaaaa's ' <payload><comment> '
                                # string
                            ...
                            # Perform the aaaaaaa's True request
                            ...
                            if trueResult and not(truePage == falsePage and not kb.nullConnection):
                                ...
                                # Perform the aaaaaaa's False request
                                if not falseResult:
                        ...
                        # In case of error-based SQL injection
                            ...
                            # Perform the aaaaaaa's request and grep the response
                            # body for the aaaaaaa's <grep> regular expression
                            try:
                        ...
                        # SQL injections
                            ...
                            # Perform the aaaaaaa's request
                            trueResult = Request.queryPage(reqPayload, place, timeBasedCompare=True, raise404=False)

                            if trueResult:
                                # Confirm aaaaaaa's results
                                trueResult = Request.queryPage(reqPayload, place, timeBasedCompare=True, raise404=False)
                        ...
                        # In case of UNION query SQL injection
                            ...
                            if not Backend.getIdentifiedDbms():
                                if kb.heuristicDbms is None:
                                    warnMsg = "using unescaped version of the aaaaaaa "
                    ...
                    # If the injection aaaaaaa was successful feed the injection
                    # object with the aaaaaaa's details
                    if injectable is True:
                        # Feed with the boundaries details only the first time a
                        # aaaaaaa has been successful
                        ...
                        # Feed with aaaaaaa details every time a aaaaaaa is successful
                        ...
                        # There is no need to perform this aaaaaaa for other
                        # <where> tags
                        break
            ...
            msg = "how do you want to proceed? [(S)kip current aaaaaaa/(e)nd detection phase/(n)ext parameter/(c)hange verbosity/(q)uit]"
...
def heuristicCheckDbms(injection):
    ...
    to identify with a simple DBMS specific boolean-based aaaaaaa what the DBMS
    may be
    """
    retVal = False
    ...
    if retVal:
        infoMsg = "heuristic (extended) aaaaaaa shows that the back-end DBMS "  # Not as important as "parsing" counter-part (because of false-positives)
...
def heuristicCheckSqlInjection(place, parameter):
    ...
    infoMsg = "heuristic (basic) aaaaaaa shows that %s parameter " % paramType
    ...
    if value in (page or ""):
        infoMsg = "heuristic (XSS) aaaaaaa shows that %s parameter " % paramType
